package com.lj.rpc.core.io.protocol.codec;

import com.lj.rpc.core.io.protocol.constans.MessageConstants;
import com.lj.rpc.core.domain.RpcMessage;
import com.lj.rpc.core.io.protocol.constans.CompressType;
import com.lj.rpc.core.io.protocol.constans.MessageType;
import com.lj.rpc.core.io.protocol.constans.SerializeType;
...
public class RpcMessageDecoder extends LengthFieldBasedFrameDecoder {
    public RpcMessageDecoder() {
        super(
                ...
                MessageConstants.MAGIC_NUMBER_LENGTH + MessageConstants.VERSION_LENGTH,
                // 描述长度的字段[4B full length（消息长度）]本身的长度，也就是 4B 啦
                MessageConstants.FULL_LENGTH_LENGTH,
                // LengthFieldBasedFrameDecoder 拿到消息长度之后，还会加上 [4B full length（消息长度）] 字段前面的长度
                // 因为我们的消息长度包含了这部分了，所以需要减回去
                -(MessageConstants.MAGIC_NUMBER_LENGTH + MessageConstants.VERSION_LENGTH + MessageConstants.FULL_LENGTH_LENGTH),
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        if(decoded instanceof ByteBuf){
            ...
            //至少包含完整的头部信息才可以进行解析
            if(frame.readableBytes() >= MessageConstants.HEADER_LENGTH){
                try {
    ...
    private RpcMessage decodeFrame(ByteBuf in) {
        ...
        int bodyLength = fullLength - MessageConstants.HEADER_LENGTH;
    ...
    private void readAndCheckVersion(ByteBuf in){
        ...
        if(version != MessageConstants.VERSION)throw new IllegalArgumentException("Unknown version:" + version);
    }
    ...
    private void readAndCheckMagic(ByteBuf in){
        byte[] bytes = new byte[MessageConstants.MAGIC_NUMBER_LENGTH];
        in.readBytes(bytes);
        for(int i = 0; i< MessageConstants.MAGIC_NUMBER_LENGTH; i++){
            if(bytes[i] != MessageConstants.MAGIC_NUMBER[i])throw new IllegalArgumentException("Unknown magic:"+ Arrays.toString(bytes));
        }
    }
